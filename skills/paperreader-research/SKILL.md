...
5. Extract ordered sections, figures, and every source-paper table. Preserve the source English PDF, extract original figure pixels, translate captions, table captions, table headers, and every table cell into Simplified Chinese, and generate the Chinese PDF with the LaTeX helpers. Empty or omitted table translations are not allowed for a completed artifact. Keep formulas, code identifiers, metric names, citations, and figure labels intact; never redraw text inside figures.
